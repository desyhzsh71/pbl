...
// get Collaborators dari organization - semua member yang active bisa lihat
export async function getCollaborators(req: Request, res: Response) {
  try {
    ...
    // mengecek apakah user adalah anggota aktif
    ...
    // mengambil semua member (termasuk owner dan pending)
...
// get - mengambil semua undangan untuk user yang login
export async function getUserInvitations(req: Request, res: Response) {
  try {
    ...
    // get semua undangan PENDING untuk user ini
...
export async function respondToInvitation(req: Request, res: Response) {
  try {
    ...
    // mengecek apakah invitation ini milik user yang login
    const member = await prisma.organizationMember.findFirst({
      where: { 
        id: memberId, 
        userId,
